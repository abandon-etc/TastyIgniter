...
## 五问已决(2026-08-31,老板/顾问答复,经用户转达)

此前悬置的五个业务问题全部有了答案,自此为**已决业务约束**,不再是待办:

| # | 问题 | 答案 |
| --- | --- | --- |
| 1 | 客户实际使用的网站要不要法语 | **必须有** |
| 2 | 法语是否默认 | **必须有,且为默认语言** |
| 3 | 系统界面文字 | **需要全法语版本** |
| 4 | 后台界面 | **暂时不用管** |
| 5 | 自动邮件 | **需要法语版** |

答案 2 与既有的承重不变量一致(`DefaultLocaleIntegrity` 守卫,PR #143 已合并):
法语是默认语言记录,列存内容以它为准。

## 重排后的工作流(2026-08-31,供确认)

| 工作项 | 原定位 | 新定位 | 依据 |
| --- | --- | --- | --- |
| **W1 界面全法语** | 待五问 | **上线必做** | 答案 1+3 |
| **W3 自动邮件法语** | 待五问 | **上线必做** | 答案 5 |
| **W2 修英文侧切换** | 先行 | **上线可选** | 法语是默认语言,顾客默认全程法语;W1 不依赖切换器。英文侧的 en/en_CA 修复留给真做英文时(它仍是 igniter-translate 对英文顾客生效的前置,该扩展本身已定为暂不装) |
| 后台翻译 | 范围内 | **整块移出范围** | 答案 4 |
| **W4 金额格式 12,00 $** | 建议做 | 建议做(不变) | 法语站的地道性,与五问独立 |

原顺序"W2 → W1 → W3 → W4"相应作废;新顺序为 **W1 → W3(并行皆可)→ W4,
W2 移出关键路径**。执行顺序与"录菜单(法语)优先"的既定安排不冲突:菜单内容
本身就是法语,录入不等任何工作项。

## W3 最小方案(2026-08-31 重估,机制已核实)

**结论:可行,而且是机制上唯一在跑的路。** 上线时全站纯法语,19 类邮件模板
**直接把存储模板的 subject/body 翻成法语**即可。

核实依据(厂商源码,只读):模板查找是
`MailTemplate::findOrMakeTemplate($code)`(`MailManager.php:82`、
`TemplateMailable.php:38`)——**按 code 单键查找,整条路径上没有任何
locale/language 维度**(`MailTemplate` 模型里 grep 不到 locale/language 字段)。
一个 code 对应一个模板,系统今天根本没有"按语言路由到第二套模板"的能力。
所以"模板语言字段"这类能力建设不是被推迟的捷径,而是双语时代才需要发明的
东西——现在做它没有任何消费者。igniter-translate 若将来安装,其
MailTemplate(subject/body)可翻译化正是那时的现成载体。

三条边界,写明免得执行时踩到:

1. **翻译动作是共享库写入**(后台编辑模板会置 `is_custom` 并把法语内容写进
   `ti_mail_templates`,对所有副本与正式站同时生效)。执行时按共享写入规矩
   报批。今天主流量 `MAIL_MAILER=log`,不会有真实顾客邮件发出,风险低,
   但性质不变。
2. **布局(layout)与模板分开存**。模板体之外,邮件布局里也可能有英文字符串,
   19 类模板翻完后要过一遍布局,否则法语正文套英文页眉。
3. 模板体内若引用 `lang()` 键,直接翻正文会把键固化为法语文本——纯法语上线
   下这是可接受的,但记住这一步之后那些键不再生效。

## W1 第一步:官方有没有 fr_CA 包 —— 取得答案的路径(2026-08-31)

| 路径 | 前置 | 状态 |
| --- | --- | --- |
| (a) 登录 tastyigniter.com 看目录页面 | 无 | **已走过:市场无语言包分类**——但市场分类不等于语言包目录的全部;hub API 的 `languages` 端点仍可能存在独立目录 |
| (b) 带 Carte Key 的一次目录调用 | Key 在手;但探测已被 Cloudflare 1010 按客户端签名封禁,带 Key 重发大概率同样被挡在 WAF,到不了鉴权层 | 不建议单独走 |
| (c) 先问官方支持解 1010 封禁 | 写一封支持邮件,说明来源 IP/UA 与持有有效 Carte | 既有首选后续,已在册 |
| (d) 0% 副本上完成绑定后从后台看语言页 | #142 已关闭;要重拾需从 CLAUDE_HANDOFF 归档重加 start.sh 块、构建副本、绑定(共享写入,须报批) | 成本最高 |

**最省事的建议:把 (c) 和一个问题合并成一封支持邮件**——既问"为何此来源被
1010 拦截",也直接问"**v4 官方目前有没有 fr_CA(或仅 fr_FR)界面语言包,
覆盖多少字符串**"。后者他们一句话就能答,答案到手连解封都可能不再急迫;
若答复只有 fr_FR,W1 的工作形态就此确定(fr_FR 为底 + 魁北克用词人工过一遍,
走已定的"覆盖源为构建输入"机制),被封禁一事变成纯粹的次要问题。
