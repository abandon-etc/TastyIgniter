...
`tools/fp1.py` is the authoritative implementation. It is deliberately not
reproduced here: two copies of an implementation drift, and a fingerprint whose
definition has drifted is worth nothing. Where the field table above and the
script disagree, the script wins and the table is the bug.

Requires Python 3 and an authenticated `gcloud` on `PATH`. It is read-only: it
describes the service and one revision and changes nothing.
